...
const ViewPaymentsCard = ({parent}) => {
  ...
  useEffect(() => {
    sethasLegalCase(
      parent?.contracts?.some(
        obj => obj.legal_case_flag == false && states.includes(obj?.state),
      ),
    );
  ...
  return (
    <Pressable
      onPress={() =>
        navigation.navigate('AllPayments', {item: parent, hasLegalCase})
      }>
      <View
        style={[
          style.flexRowbtw,
...
export default ViewPaymentsCard;
