...
const ViewContractCard = ({parent}) => {
  const navigation = useNavigation();
  const {language, switchLanguage} = useLanguage();
  const {isDark} = useSelector(state => state.Home);

  return (
    <Pressable onPress={() => navigation.navigate('Contracts', {item: parent})}>
      <View
        style={[
          style.flexRowbtw,
        ...
        <View
          ...
          <View style={[style.flexcolstart, {backgroundColor: 'transparent'}]}>
            <Text
              numberOfLines={1}
              style={{
...
export default ViewContractCard;
