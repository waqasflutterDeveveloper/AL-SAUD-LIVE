...
const Contracts = ({route}) => {
  ...
  // console.log(parent?.contracts[0]?.prints, 'arent?.contracts');
  return (
    <SafeAreaView
      style={{
      ...
      />
      <Header title={'My Contracts'} back />
      <FlatList
        snapToInterval={width - 20}
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={{marginVertical: 10, marginHorizontal: 10}}
        vertical
        data={parent?.contracts}
        renderItem={({item}) => (
          <ContractCard navigate={true} parent={parent} item={item} />
        )}
